    ...
    public static float[] generateHeightMap(int width, int height, float min, float max, int octaveCount) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                bytes[idx++] = (noise2(0, x, y) * range + min);
            }
        }
        return bytes;
    }

    public static Pixmap generatePixmap(int width, int height, float min, float max, int octaveCount) {
        float[] bytes = generateHeightMap(width, height, min, max, octaveCount);
        Pixmap pixmap = new Pixmap(width, height, Pixmap.Format.RGBA8888);
        ByteBuffer buffer = pixmap.getPixels();

        for (int i = 0, idx = 0; i < bytes.length; i++) {
            byte val = (byte) (bytes[i] * 256);
            buffer.put(idx++, val);
            buffer.put(idx++, val);
            buffer.put(idx++, val);
